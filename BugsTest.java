...
"}\n");
		 parser.isProgram();
		interpret = new Interpreter(parser.stack.peek());
		bugs = interpret.bugs.get("Diamond");
	}
	...
	public void testConstructor(){
		...
		assertTrue(bugs.name.equals("Diamond"));
	...
	public void testLoop(){
		...
		bugs.scopes.push(bugs.myVariables);
...
//		bugs.scopes.push(bugs.myVariables);
	...
	public void testCall(){
		parser = new Parser("define abc using a, b {\n move a+b \n return a + b \n}\n");
		parser.isFunctionDefinition();
		bugs = new Bugs();
		bugs.interpret(parser.stack.peek());
		bugs.myFunctions.containsKey("abc");
		parser = new Parser("abc (p, q)");
		bugs.myVariables.put("p", 3.0);
		bugs.myVariables.put("q", 6.0);
		bugs.scopes.push(bugs.myVariables);
		parser.isFunctionCall();
		assertEquals(bugs.evaluate(parser.stack.pop()), 9.0, 0.01);
		bugs.myFunctions.containsKey("abc");
		assertTrue(bugs.myVariables.get("p") == 3.0);
		assertTrue(bugs.myVariables.get("q") == 6.0);
